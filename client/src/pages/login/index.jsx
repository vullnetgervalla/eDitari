...
function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { userid, schoolid, userType, accessToken } = response?.data;
            setAuth({ email, userid, schoolid, userType, accessToken });
            if (persist) {
    ...
    return (
        <div className="flex h-screen overflow-hidden w-full items-center justify-end" style={{ backgroundImage: 'url(/images/login.jpg)', backgroundSize: 'cover', backgroundPosition: 'right' }}>
            <div className="h-full flex flex-col items-center justify-center bg-white p-8 shadow-lg light:bg-gray-800" style={{ backgroundColor: '#F8F8FF', minWidth: '31%' }}>
